...
export default function App() {
  ...
  for (let i = 0; i < list1.length; i++) {
    tmp.push(<FriendReceived key={i} name={list1[i].name} img={list1[i].img} />);
  }
  let tmp1 = [];
  for (let i = 0; i < list2.length; i++) {
    tmp1.push(<FriendSent key={i} name={list2[i].name} img={list2[i].img} />);
  }
  ...
  return (
    ...
    >
      <Tab.Screen
        ...
        options={{ tabBarLabel: 'ĐÃ NHẬN ' + list1.length}}
      ...
      <Tab.Screen
        name="ListSent"
        component={ListSent}
        options={{ tabBarLabel: 'ĐÃ GỬI ' + list1.length}}
      />
